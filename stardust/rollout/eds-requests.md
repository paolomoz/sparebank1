...
## W7 (category-hub siblings at 360 — forsikring · pensjon · sparing · lofavor · tips-og-rad · daglig-bruk · eiendom) — 2026-09-15

Everything is mirrored as additive `@media (width <= 767px)` rules (styles.css "category-hub (W7 mobile)" block; cards/banner/accordion/columns.css W7 blocks) plus two general cards.css fixes; the canon-level change is requested here.

1. **Card `height: 100%` inside a grid `li` that also holds the card's 16px margin** (`.cards.grid .card { margin-bottom: 16px }` + `height: 100%`): the card absorbs its own margin and the flex/grid content stretches by 16px (boxed 403 vs live 387, illustrated 332 vs 310, plain 128 vs 110, featured text cards +16). Fixed at 360 with `height: auto`; at 1440 the same +16 exists in every row of boxed cards (forsikring boxes sit 19px low — W6 residual) — the core model should size the li, not the card.
2. **`.cards.boxed .card__expand--inline` inherited `width: 100%` from `.card__expand`** → the "Se alle ⌄" expander was stretched and left-aligned at every width (live centres it: x195 in a 240 box at 1440, x135 in 280 at 360). Fixed generally (`width: auto; gap: 8px`); ditto `.cards.boxed .card__media img { object-position: 50% 50% }` (the 280×150 SVG keeps xMidYMid meet inside the 200px img — the generic photo top-anchor was wrong). These two move the forsikring 1440 gate 8.23 → 8.37 % (Δh −29 unchanged, same bands) because the boxes are 19px low at 1440 and the corrected centring no longer cancels that.
3. **Paragraph strut for `span.subtle-text`**: 14px glyphs on the enclosing 16/24 line box (2 lines = 48) — the whole-paragraph cases (hero note, box texts, FAQ note, Fremtind text, photo caption) should carry `line-height: 24px`, not 20, at both widths (W6's 20 + 4px `::after` modelled the 1-line 1440 case only).
4. **`text-wrap: balance` on live h1/h2** (forsikring "Hvilke forsikringer / passer for meg?", "Blant Norges beste / på skadeoppgjør", "Forsikringsselskapet / vårt er Fremtind"; pensjon "Sjekk pensjonen din og / se hvor mye mer pensjon / du kan få…"; lofavor "Er du medlem i / et LO-forbund?") and `text-wrap: pretty` on nav-card paragraphs (sparing "…godt liv / som pensjonist") — the canon has `text-wrap: initial` on headings. Applied at ≤767 only.
5. **Inline-SVG slack**: live illustration images are inline `img.inline-svg` with ~6–7px descender slack under them (forsikring calculator illustration, standalone illustration, media-plain images; pensjon question mark). The columns block renders block pictures. Applied per layout token at 360 (`lg-4-offset-1-first-w600`, `lg-12-w175`, `media-plain .col--media`, `lg-3-offset-0-first-w180`); the encoder should emit a media token for illustrations even after the SVG→PNG rewrite so one rule can carry it.
6. **Visual-nav text cards (`cards nav`, W1)**: live cards are content-sized (1-line text 114, 2-line 137) with 24 between them (lån pitches 139/162/162, sparing 162); W1's fixed 114 + 48 matched only the 2-line pitch. Fixed at 360 (lån 360: 2.09 → 1.80 %, Δh 0); the 1440 rule (`height: 120px`) is probably the same class.
7. **Live 360 module gaps the hub skins carried from 1440**: `hub-static` after a pill 48 (not 72); `hub-title-lead` under the bank-choice 48 (not 72) with the lead spanning 320; nested wrap padding 16 (not 24); `hub-center` sub-lead: plain 16/24 in a ≤242px column on forsikring (`band`), 17/24 full width on pensjon (`cols`) — two live classes under one token; news-rail heading → card 17 (mb 16); FAQ band content x24–335; `hub-close` rule → heading 64.
8. **Two-tone banner (`banner columns`, market-landing) at 360**: the tinted column has no top padding — the illustration hangs 60px above the tint (block padding-top 54 instead of column padding); the pill sits 28 under the text / 8 above the padding (`justify-content: flex-start` in the column-reverse bottom).
9. **Pills/expanders with wrapping labels** grow on live (68px): `.disclosure__btn`, `.card__expand` (boxed secondary), hero CTAs stretch to the 320px column (`hub-cta`). Core buttons are fixed 44px.
10. **`rule-visible` at 360**: the live hr module is inset by the section padding (x20–339; x16–343 in bands) and the feedback question sits 68 under it; the section border spans full width. Applied as an inset `::before` for the feedback and `hub-close` cases only.
11. **Savings-calculator snapshot (`data/calculator/sparing-360.html`, W6, shadow root)**: the results panel has 33px less between the "Se avkastning med ulik risiko" pill and the footnote than live (43 vs 63+), 24px shorter overall — a snapshot-file fix (block CSS cannot reach the shadow root). Sparing still gates at Δh 0 because the rest of the page absorbs it.
12. **Rasterised SVG canvases**: `rasterise-svg` output carries more padding than the SVG viewBox (tips-og-rad boxes: ink 196×107 vs live 212×115 in the same 248px image) — ~5px per box, visible.
13. **Content-level 360 residuals seen in the live captures**: pensjon blank `<p></p>` spacer (56px, recorded); lofavor's live has no breadcrumb row at 360 while the document (and tips-og-rad's live) has one; the pensjon feedback question is left-aligned on live (x21) vs the block's centred p; sparing calculator default "Moderat" vs snapshot "Ingen".
14. Live hits this pass: 0 (all measurements from the cached 360 captures, W6's live DOM dumps and the emulation).

- Orchestrator 2026-09-15: forsikring 360 published Δh 25 vs emulation Δ4 — three ~6–7px steps at inline illustrations (barn_spiller_fotball columns row, 'Blant Norges beste', 'Snarveier'): the pipeline's <picture><source…><img> renders a different line box than the emulation's wrapper for external images. serve.mjs must wrap EVERY external <img> exactly like the pipeline (sources + intrinsic width/height) — Flow B L7 class. Until then, treat ≤ 7px per inline illustration as a known emulation delta.
