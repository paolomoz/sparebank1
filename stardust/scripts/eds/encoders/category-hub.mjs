...
export function hubBand(root, ctx, { topLevelCols = false } = {}) {
  ...
  const walk = (el) => {
    for (const ch of el.children) {
      ...
      const encKey = k.find((c) => !['band', 'cols', 'richtext', 'image', 'module', 'banner-small', 'related-products', 'related-topics'].includes(c) && (OWN[c] || CORE[c])); // related-* take the hubRelated branch below (core relatedTopics reads only .newsfeed → 0 rows, silently) // prose-shaped children keep the richtext path below // a module inside the band (faq, tip, prices…): its encoder's parts, inlined
      if (encKey) {
