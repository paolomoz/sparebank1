...
export function omBand(root, ctx, opts = {}) {
  ...
  const encodeRow = (row) => {
    ...
    const variants = cols.map((c) => { const k = cls(c); const span = (k.find((x) => /^col-lg-\d+$/.test(x)) || 'col-lg-12').replace('col-', ''); const off = k.find((x) => /^col-lg-offset-\d+$/.test(x)); const first = k.includes('col--first') ? 'first' : null; const align = (k.find((x) => /^col--(middle|center|bottom)$/.test(x)) || '').replace('col--', ''); const rts = qa(c, ':scope > .col__content > .richtext'); const split = rts.length >= 2 && rts.every((r) => r.children.length === 1 && r.children[0].tagName === 'P') ? 'split' : null; // several one-paragraph live text modules (their 16px margins stack)
      return [span, off ? off.replace('col-lg-offset-', 'offset-') : null, first, align, illustrationWidth(c), split].filter(Boolean).join('-'); });
    ...
    if (qa(row, '.subtle-text').some((s) => s.closest('p'))) variants.push('subtle'); // a subtle-text PARAGRAPH (presse) — not a span inside a heading (markedsnytt expert h4)
    ...
    const tmax = /--max:\s*(\d+)px/.exec(qa(row, '.col__content > .richtext').map((r) => r.getAttribute('style') || '').join(' '))?.[1]; if (tmax && opts.textMax !== false) variants.push(`text-${tmax}`); // live richtext--max text column (theme; markedsnytt's CSS ignores --max)
