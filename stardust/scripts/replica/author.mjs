...
function moduleOf(node) {
  ...
  switch (k) {
    ...
    case 'usp': { const s = el('section', { class: 'usp' }); for (const item of node.querySelectorAll('.usp-item')) { const it = el('div', { class: 'usp__item' }); const svg = item.querySelector('svg'); if (svg) it.append(el('span', { class: 'usp__icon' }, [svgOf(svg)])); for (const ch of item.childNodes) { if (ch.nodeType === 1 && ch.tagName === 'svg') continue; const cc = cleanCopy(ch); if (cc) it.append(cc); } s.append(it); } return s; }
