...
export default {
  ...
  'related-topics': (node, ctx) => {
    ...
    for (const c of node.querySelectorAll('.card')) {
      ...
      if (ctx.family === 'market-landing' || ctx.family === 'markedsnytt-listing' || ctx.family === 'category-hub') { // category-hub added by W6 (sparing top-level rail: live .card__date) // both families carry the live .card__date (registry key owned here; markedsnytt-listing.mjs sorts before this file)
